...
void display() {  // Procedure untuk menampilkan hasil
	...
	cout << "=================================" << endl;    // Output ke layar
	for (int j = 0; j < n; j++) {               //Looping dengan j dimulai daro 0 hingga n-1
		cout << a[j] << endl;                   // Output ke layar
	}
	cout << endl;                               // Output basir kosong
} 

void bubbleSortArray() {     // Procedure untuk mengurutkan array dengan metode bubble Sort
	for (int i = 1; < n; i++) {      //Looping dengan i dimulai dari 1 hingga n-1
		for (int j = 0; j < n - i; j++) {        // Looping dengan j dimulai dari n-i-1 
			if (a[j] > a[j + 1]) {          // Jika nilai pada a[j] lebih besar dari a[j+1]
				int temp = a[j];            // Simpan nilai a[j] ke variabel sementara temp
				a[j] = a[j + 1];            // Assign nilai a[j+1]
